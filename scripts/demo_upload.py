...
COMMON COMMANDS:
  ...
  python3 demo_upload.py doctor --url <demo-url>   Preflight: token+TTL, demo URL, workspace, roster
  ...
  python3 demo_upload.py upload demo.json --url <demo-url> --replace  Replace ALL actions instead of appending
  python3 demo_upload.py upload demo.json --url <demo-url> --replace-channel <name>  Replace only one channel's actions
...
HUMAN-FRIENDLY UPLOAD FORMAT (the CLI resolves these to IDs for you):
  ...
  - "text":        plain text, OR a serialized Block Kit JSON string {"blocks":[...]} for rich app cards
...
# Schema source of truth: reference/SCHEMA_conversations_actions.md
...
def parse_blockkit(text):
    """If `text` is a serialized Block Kit JSON string, return the parsed object;
    otherwise return None. Rich app cards are stored as a json.dumps'd
    {"blocks":[...]} string placed in the action's `text` field (the demo-zone
    schema has no dedicated blocks field)."""
...
def validate_demo(demo):
    """Validate against SCHEMA_conversations_actions.md. Returns a list of
    ...
    for i, a in enumerate(actions):
        ...
        # Text required for Message/Thread. May be plain text OR a serialized
        # Block Kit JSON string — if it parses as JSON it must have a 'blocks' array.
        if atype in TYPES_NEED_TEXT:
            text = a.get("text")
            if not (text or "").strip():
                errors.append(f"{where} ({atype}): 'text' is required and must be non-empty.")
            else:
                bk = parse_blockkit(text)
                if bk is not None:
                    blocks = bk.get("blocks")
                    if not isinstance(blocks, list) or not blocks:
                        errors.append(f"{where} ({atype}): 'text' looks like Block Kit JSON but "
                                      f"has no non-empty top-level 'blocks' array.")
...
def cmd_doctor(workspace=None, url=None):
    """Preflight everything the build depends on, in order, failing fast with a
    clear message. Run this BEFORE spending time authoring conversations."""
    print("Running preflight checks...\n")

    # 1. Token present + valid + TTL
    token = keychain_load()
    if not token:
        sys.exit("✗ No token saved. Run: python3 demo_upload.py login")
    if token_is_expired(token):
        sys.exit("✗ Token expired. Run: python3 demo_upload.py login")
    remaining = jwt_exp(token) - int(time.time())
    mins = remaining // 60
    flag = "  ⚠️  refresh soon" if remaining < 600 else ""
    print(f"✓ Token valid — expires in ~{mins} min{flag}")

    # 2. Demo URL / workspace resolve
    if not workspace and not url:
        sys.exit("✗ Provide --url <demo-url> or --workspace <uid> to check workspace + roster.")
    try:
        ws, demo_id = resolve_workspace(token, workspace=workspace, url=url)
    except SystemExit as e:
        raise
    print(f"✓ Workspace resolves — {ws}")
    if demo_id:
        print(f"✓ Demo reachable — {demo_id}")

    # 3. Roster non-empty
    users = api_get(f"workspace/{ws}/users?include_connect_users=true", token)
    bots  = api_get(f"bots?workspace_id={ws}", token)
    if not users:
        sys.exit("✗ Roster has no users — wrong workspace?")
    print(f"✓ Roster — {len(users)} users, {len(bots)} bots")

    print("\nAll checks passed. Safe to author + upload.")
...
def cmd_create_channel(workspace_uid, name, invites, token, is_private=False):
    ...
    Duplicate guard: if a channel with this name already exists, skip + report
    rather than creating a second one (re-running used to make dupes)."""
    existing = load_channels(workspace_uid, token)
    if name.lower() in existing:
        print(f"Channel #{name} already exists ({existing[name.lower()]}) — skipping creation.")
        return
    payload = {
        "name": name,
        "topic": "",
...
def cmd_upload(json_path, workspace_uid=None, demo_url=None, dry_run=False,
               replace=False, replace_channel=None):
    path = Path(json_path)
    if not path.exists():
        sys.exit(f"File not found: {json_path}")
    try:
    ...
    if replace_channel:
        # Replace only the actions belonging to one channel; keep everything else.
        # Resolve the channel name to an ID so we match against existing actions.
        target_id = resolve(replace_channel, channels, "channel")
        kept = [a for a in existing_actions if a.get("channel") != target_id]
        prepared["conversations_actions"] = kept + new_actions
        removed = len(existing_actions) - len(kept)
        print(f"Replacing channel '{replace_channel}': removed {removed} existing action(s), "
              f"adding {len(new_actions)} new (kept {len(kept)} from other channels).")
    elif replace:
        prepared["conversations_actions"] = new_actions
        print(f"Replacing ALL {len(existing_actions)} existing action(s) with {len(new_actions)} new.")
    else:
...
def main():
    ...
    dr = sub.add_parser("doctor", help="Preflight: token+TTL, demo URL, workspace, roster")
    add_ws(dr)
    ...
    up.add_argument("--replace", action="store_true",
                    help="Replace ALL the demo's actions instead of appending to them")
    up.add_argument("--replace-channel", metavar="NAME",
                    help="Replace only the actions for this channel; keep all other channels")
